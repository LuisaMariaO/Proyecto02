...
function CrearUsuario(){
    ...
    fetch(`http://104.154.88.173:5000/pacientes/${usuario.value}`) //Buscando usuarios repetidos
    ...
    fetch('http://104.154.88.173:5000/registro',
    {
        method:'POST',
        headers,
...
function IniciarSesion(){
    ...
    if(usuario.value=="" && pass.value==""){
        ...
        fetch(`http://104.154.88.173:5000/login/${usuario.value}/${pass.value}`)
        // Convirtiendo de string a texto
        .then(response => response.json())
        .then(data => {
            
            if(data.nombre=="false"){
                fetch(`http://104.154.88.173:5000/loginmedico/${usuario.value}/${pass.value}`) //Buscando doctores
                // Convirtiendo de string a texto
                .then(response => response.json())
                .then(data => {
                    
                    if(data.nombre=="false"){
                        fetch(`http://104.154.88.173:5000/loginenfermera/${usuario.value}/${pass.value}`) //Buscando enfermeras
                        // Convirtiendo de string a texto
                        .then(response => response.json())
                        .then(data => {
                            ...
                            alert("Verifique sus credenciales")
                                ...
                                fetch(`http://104.154.88.173:5000/setlogenfermera/${usuario.value}`, {
                                    method: 'POST',
                                    headers,
                                    body: `{
                                        "user":"${usuario.value}"
                        ...
                        fetch(`http://104.154.88.173:5000/setlogmedico/${usuario.value}`, {
                            method: 'POST',
                            headers,
                            body: `{
                                "user":"${usuario.value}"
                ...
                alert(`Bienvenido ${data.nombre}`)
    fetch(`http://104.154.88.173:5000/setloguser/${usuario.value}`, {
    method: 'POST',
    headers,
    body: `{
        "user":"${usuario.value}"
